...
public class UpdateUserCommand implements Command {
	...
	// update-user -–session-id <session-id> -–new-username <newUsername>
	// --new-first-name <newFirstName> --new-last-name <newLastName> --new-email
	// <email>. Everything except --session-id is optional.
	...
	public CommandResult execute(String[] commandOptions) {
		...
		updateUserResult.setResultMessage(String.format(confirmationMessage, userToUpdate.getUsername(),
				newSession.getID(), newSession.getTimeToLive()));
		return updateUserResult;
	}
	...
	private static void updateUserDetailsForUserAccordingOptions(User userToUpdate, String[] commandOptions) {
		...
		for (int i = 0; i < commandOptions.length; i += 2) {
			...
			case "-–new-username":
				userToUpdate.setUsername(optionValue);
				break;
			case "--new-first-name":
				userToUpdate.setFirstName(optionValue);
				break;
			case "--new-last-name":
				userToUpdate.setLastName(optionValue);
				break;
			case "--new-email":
				userToUpdate.setEmail(optionValue);
				break;
